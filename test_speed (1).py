...
def test_image_tokens():
    ...
    for width, height in test_sizes:
        ...
        # No padding, no truncation — just count raw tokens
        inputs = processor(
            text=text,
            images=img,
            return_tensors="pt",
    ...
    print("\n  Summary:")
    print("  - 336x336 produces ~1365 tokens (single patch)")
    print("  - Anything larger produces ~3117 tokens (multi patch)")
    print("  - Recommended: resize to 336x336, use max_length=1536")

    return processor


def test_training_speed(processor):
    """Run a quick 10-step training at different settings to measure speed."""
    print("\n" + "=" * 60)
    ...
    # Key: image size determines token count, max_length must fit token count
    configs_to_test = [
        {"label": "OPTIMIZED (resize 336)", "image_size": (336, 336), "max_length": 1536, "batch_size": 4},
        {"label": "OPTIMIZED + bigger batch", "image_size": (336, 336), "max_length": 1536, "batch_size": 8},
        {"label": "CURRENT (no resize)", "image_size": (672, 672), "max_length": 3200, "batch_size": 4},
    ]
    ...
    for cfg in configs_to_test:
        ...
        print(f"\n--- {cfg['label']}: image={img_size}, max_length={max_len}, batch={batch_sz} ---")
        ...
        try:
            # Process one example with padding to max_length
            inputs = processor(
                text=text,
                images=img,
                return_tensors="pt",
            ...
            token_count = inputs["input_ids"].shape[1]
            print(f"  Padded token count: {token_count}")
            ...
            per_step = elapsed / 10
            print(f"  10 steps in {elapsed:.1f}s ({per_step:.2f}s/step)")
            ...
            print(f"  Peak GPU memory: {mem_used:.1f} GB / 24 GB")

            torch.cuda.reset_peak_memory_stats()

        except RuntimeError as e:
            if "out of memory" in str(e).lower():
                print(f"  OUT OF MEMORY — this config doesn't fit on A6000 24GB")
                torch.cuda.empty_cache()
            else:
                print(f"  ERROR: {e}")
        except ValueError as e:
            print(f"  VALUE ERROR: {e}")
            print(f"  This means max_length is too small for the token count")

    print("\n" + "=" * 60)
    print("SUMMARY: Pick the fastest config that fits in memory")
    print("=" * 60)
