...
def list_frame_files(client, bucket, folder_prefix):
    ...
    Returns a dict: { timestamp_string: filepath }
    
    Example:
      "2024-05-21 16-06-29,305" -> "workobject1_frames/2024-05-21 16-06-29,305_normal_0.jpg"
    """
    frames = {}
    ...
    for obj in objects:
        ...
        # Format: "2024-05-21 16-06-29,305_normal_0.jpg" or "2024-05-21 16-06-29,305_pore_0.jpg"
        # Timestamp part is everything before _normal_ or _pore_
        match = re.match(r"^(.+?)_(normal|pore)_(\d+)\.jpg$", filename)
        if not match:
            continue

        timestamp_str = match.group(1)  # "2024-05-21 16-06-29,305"
        label = match.group(2)          # "normal" or "pore"
        index = int(match.group(3))     # 0, 1, etc.

        # Key: (timestamp, index) to handle duplicate timestamps
...
def excel_timestamp_to_filename_format(ts):
    ...
    if isinstance(ts, str):
        # Already a string, just convert format
        ts_str = ts
    ...
    # Replace colons with hyphens (time part)
    # "2024-05-21 16:06:29.305" -> "2024-05-21 16-06-29.305"
...
def load_workobject_data(client, bucket, excel_path, frame_folder, input_columns, 
                          label_column, timestamp_column):
    """
    ...
    [
        {
            "image_path": "workobject1_frames/2024-05-21 16-06-29,305_normal_0.jpg",
            "prompt": "Analyze this Wire DED sensor reading...",
            "target": "NORMAL — no porosity detected.",
            "pore_diameter": 0.0,
